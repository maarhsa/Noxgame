...
global $lang, $ProdGrid, $resource, $reslist, $game_config, $_POST;
	...
	foreach($reslist['prod'] as $ProdID) {
		if ($CurrentPlanet[$resource[$ProdID]] > 0 && isset($ProdGrid[$ProdID])) {
			...
			if($user['rpg_general']!=0)
				...
				{
					$pourcentbonusmetal =floor( eval ( $ProdGrid[$ProdID]['formule']['metal']     ) * ( $game_config['resource_multiplier'] )* 0.1);
					$pourcentbonuscristal =floor( eval ( $ProdGrid[$ProdID]['formule']['crystal']   ) * ( $game_config['resource_multiplier'] )* 0.1);
					$pourcentbonusdeut =floor( eval ( $ProdGrid[$ProdID]['formule']['deuterium'] ) * ( $game_config['resource_multiplier'] )* 0.1);
				...
				$metal     = (floor( eval ( $ProdGrid[$ProdID]['formule']['metal'] )     * (0.01 * $post_porcent) * ( $game_config['resource_multiplier'] ))+ $pourcentbonusmetal)*$bonusalli;
				$crystal   = (floor( eval ( $ProdGrid[$ProdID]['formule']['crystal'] )   * (0.01 * $post_porcent) * ( $game_config['resource_multiplier'] ))+ $pourcentbonuscristal)*$bonusalli;

				if ($ProdID < 4)
					{
						$deuterium = (floor( eval ( $ProdGrid[$ProdID]['formule']['deuterium'] ) * (0.01 * $post_porcent) * ( $game_config['resource_multiplier'] ))+ $pourcentbonusdeut)*$bonusalli;
						$energy   =  (floor( eval  ( $ProdGrid[$ProdID]['formule']['energy']) * ( $game_config['resource_multiplier']) * ( 1 + ( $CurrentUser['energy_tech'] * 0.01 ) ))+ $pourcentbonusenergie)*$bonusalli;
					}
					elseif ($ProdID >= 4 )
					{
	...
	$parse['daily_deuterium']       = floor($CurrentPlanet['deuterium_perhour']* 24      * 0.01 * $parse['production_level'] + $parse['deuterium_basic_income']* 60 * 24      );
