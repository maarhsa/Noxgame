...
if(isset($_GET['mode'])){
	...
	$page .= "</th></tr>";
	$page .= '<tr><td colspan=2 class=c><a href="'. INDEX_BASE .'raccourci">Effacer</a></td></tr></tr></table></form>';
}
elseif(isset($_GET['a'])){
	if($_POST){
		//Armamos el array...
		...
		if($_POST["delete"]){
			...
			doquery("UPDATE {{table}} SET fleet_shortcut='{$user[fleet_shortcut]}' WHERE id={$user[id]}","users");
			message("Shortcut wurde gel&ouml;scht","Gel&ouml;scht","". INDEX_BASE ."raccourci");
		}
		else{
			$r = explode(",",$scarray[$a]);
	...
	<tr height="20">
	<td colspan="2" class="c">Raccourcis(<a href="'. INDEX_BASE .'raccourci&mode=add">Ajout</a>)</td>
	</tr>';
	...
	if($user['fleet_shortcut']){
		...
		*/
		...
		foreach($scarray as $a => $b){
			...
			$page .= "<th><a href=\"". INDEX_BASE ."raccourci&a=".$e++."\">";
	...
	$page .= '<tr><td colspan=2 class=c><a href="'. INDEX_BASE .'flotte">Retour</a></td></tr></tr></table>';
}
