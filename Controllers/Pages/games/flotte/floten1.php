 ...
 */
	...
	if($i >= 200 && $i <= 300)
	{
		for($i = 200;$i < 300 ;$i++)
		{
			$shipchiffres = preg_match("/[^0-9]/", $_POST["ship$i"]);		
			// si il y a un petit malin qui rentre des caracteres autre que numerique				
			if($shipchiffres) message("".$lang['error']."","".$lang['error_g']."");
		}
	}
	...
	if($user[$resource[115]] >=1)
	{
		...
		{
			message("".$lang['error']."","Vous etes Occulté !!!");	
		}
	}

	
	elseif (!$fleet['fleetlist']) {
	...
	$page .= "}\n";
	$page .= "</script>\n";
	$page .= "<form action=\"". INDEX_BASE ."depart\" method=\"post\">";
	...
	{
	$page .= "<tr height=\"20\">";
	$page .= "<th>Propulsion</th>";
	$page .= "<th>";
	$page .= "<select name=\"propuls\" onChange=\"shortInfo()\" onKeyUp=\"shortInfo()\">";
	foreach ($vitesse as $a => $b) {
		$page .= "<option value=\"".$a."\">".$b."</option>";
	}
	$page .= "</select>";
	$page .= "</th>";
	...
	// if($verification_techno['hyperspace_tech']>=5)
	// {
	// $page .= "<th><div id=\"durationHyper\">-</div></th>";
	// }
	// else
	// {
...
*/
	...
	$page .= "<tr height=\"20\">";
	$page .= "<td colspan=\"2\" class=\"c\">". $lang['fl_shortcut'] ." <a href=\"". INDEX_BASE ."raccourci\">". $lang['fl_shortlnk'] ."</a></td>";
	$page .= "</tr>";
