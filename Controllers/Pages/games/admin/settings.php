...
function DisplayGameSettingsPage ( $CurrentUser ) {
	...
	if ( $CurrentUser['authlevel'] == 3) {
		if ($_POST['opt_save'] == "1") {
			...
			// BBCode ou pas ?
			if (isset($_POST['bbcode_field']) && $_POST['bbcode_field'] == 'on') {
				$game_config['enable_bbcode']         = "1";
			} else {
				$game_config['enable_bbcode']         = "0";
			}
			
			// inscription ou pas ?
			if (isset($_POST['reg_valid']) && $_POST['reg_valid'] == 'on') {
				$game_config['enable_reg']         = "1";
			} else {
				$game_config['enable_reg']         = "0";
			}
			
			// frames ou pas ?
			if (isset($_POST['frame_valid']) && $_POST['frame_valid'] == 'on') {
				$game_config['frame_disable']         = "1";
			} else {
				$game_config['frame_disable']         = "0";
			}
			
			// tchat overview ou pas ?
			if (isset($_POST['tchat_valid']) && $_POST['tchat_valid'] == 'on') {
				$game_config['enable_tchat_over']         = "1";
			} else {
				$game_config['enable_tchat_over']         = "0";
			}
			
			// Y a un News Frame ? !
			if (isset($_POST['newsframe']) && $_POST['newsframe'] == 'on') {
				$game_config['OverviewNewsFrame']     = "1";
				$game_config['OverviewNewsText']      = html_entity_decode($_POST['NewsText']);
			} else {
            ...
            // Revenu de base E2PZ
			if (isset($_POST['materia_basic_income']) && is_numeric($_POST['materia_basic_income'])) {
				$game_config['materia_basic_income'] = $_POST['materia_basic_income'];
			}
...
$game_config['banner_source_post'] = $_POST['banner_source_post'];
	...
	if (isset($_POST['bot_enable']) && is_numeric($_POST['bot_enable'])) {
			...
			//activation l'inscription
			doquery("UPDATE {{table}} SET `config_value` = '". $game_config['enable_reg']           ."' WHERE `config_name` = 'enable_reg';", 'config');
			
			//activation pour frames
			doquery("UPDATE {{table}} SET `config_value` = '". $game_config['frame_disable']           ."' WHERE `config_name` = 'frame_disable';", 'config');			
			//activation tchat de l'overview
			doquery("UPDATE {{table}} SET `config_value` = '". $game_config['enable_tchat_over']           ."' WHERE `config_name` = 'enable_tchat_over';", 'config');
			...
			$parse['bbcode_field']           = ($game_config['enable_bbcode'] == 1) ? " checked = 'checked' ":"";

			$parse['reg_valid']           = ($game_config['enable_reg'] == 1) ? " checked = 'checked' ":"";
			$parse['frame_valid']           = ($game_config['frame_disable'] == 1) ? " checked = 'checked' ":"";
			
			$parse['tchat_valid']           = ($game_config['enable_tchat_over'] == 1) ? " checked = 'checked' ":"";
