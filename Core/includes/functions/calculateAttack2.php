...
function calculateAttack2 (&$attackers, &$defenders) 
{
	...
	//points de structure du defenseur
		...
		for ($round = 0, $rounds = array(); $round < $max_rounds; $round++) {
			...
			foreach ($attackers as $fleetID => $attacker) {
				...
				foreach ($attacker['detail'] as $element => $amount) {
					...
					// on ajouter le lazer,ions et plasma ^^
					$lazer    = $amount * ((1 + (0.05 * ($attacker['user']['laser_tech']))) * (rand(80, 120) / 100));
					
					if($element == 206 or $element == 207 or $element == 211 or $element == 213 or $element == 214 or $element == 215)
					{
						$ions = $amount * ((1 + (0.05 * ($attacker['user']['ionic_tech']))) * (rand(80, 120) / 100));
					}
					else
					{
						$ions = 1;
					...
					if($element == 211 or $element == 213 or $element == 214 or $element == 215)
					{
						$plasma	= $amount * ((1 + (0.05 * ($attacker['user']['buster_tech']))) * (rand(80, 120) / 100));
					}
					else
					{
						$plasma = 1;
					...
					$attackDefens[$fleetID] += $thisDef;
					$attackDefens['total'] += $thisDef;
			...
			foreach ($defenders as $fleetID => $defender) {
				...
				foreach ($defender['def'] as $element => $amount) {
					...
					$defenseDefens[$fleetID] += $thisDef;
					$defenseDefens['total'] += $thisDef;
...
// UGLY CODE!! WARNING
				...
				foreach($attacker['detail'] as $element => $amount) {
					...
					if ($amount > 0) {
						if ($attArray[$fleetID][$element]['shield']/$amount < $defender_moc) {
							...
							$ile_removePoints = floor($defender_moc / (($pricelist[$element]['metal'] + $pricelist[$element]['crystal'])  / 5));
						...
						$nbTirsAtt[$round]= $amount * (rand(100,120)/100);
					} else {
						$attacker_n[$fleetID][$element] = round($amount);
						$attacker_shield += $defender_moc;
					}
				...
				foreach($defender['def'] as $element => $amount) {
					...
					if ($amount > 0) {
						if ($defArray[$fleetID][$element]['shield']/$amount < $attacker_moc) {
							...
							$ile_removePoints = floor(($attacker_moc / ((($pricelist[$element]['metal'] + $pricelist[$element]['crystal']) / 5) )));
           ...
           // "Rapidfire"
			...
			$rounds[$round]['nbtiratt'] = $nbTirsAtt[$round];
			$rounds[$round]['nbtirdef'] = $nbTirsDef[$round];
