...
function FinishshipFromQueue(&$CurrentPlanet,$CurrentUser) 
{
	...
	if($CurrentUser['vote']>= MAX_FINISH_BONUS_FLEET)
	{
		...
		$Estado = "Vaisseaux";
		$Text = "".$user['username']." a finis la construction  du vaisseaux :".$lang['tech'][$Element]." avec la quantité :".$nombre." prematurement le ".date ( "d-m-Y H:i:s" , time() )." \n";
		$log=LogFunction($Text ,$Estado ,1);
		
		header("Location:". INDEX_BASE ."chantier");
	}
	return $ReturnValue;
}
