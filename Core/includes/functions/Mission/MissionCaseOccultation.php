 ...
 *
 */
function MissionCaseOccultation ( $FleetRow ) {
	...
	if ($FleetRow['fleet_mess'] == 0) {
		...
		if ($iGalaxyPlace == 0) {
			...
			if ($iPlanetCount >= MAX_PLAYER_PLANETS) {
				$TheMessage = $lang['sys_occult_arrival'] . $TargetAdress . $lang['sys_occult_maxcolo'] . MAX_PLAYER_PLANETS . $lang['sys_occult_planet'];
				SendSimpleMessage ( $FleetRow['fleet_owner'], '0', $FleetRow['fleet_start_time'], 0, $lang['sys_occult_mess_from'], $lang['sys_occult_mess_report'], $TheMessage);
				doquery("UPDATE {{table}} SET `fleet_mess` = '1' WHERE `fleet_id` = ". $FleetRow["fleet_id"], 'fleets');
			} else {
				$NewOwnerPlanet = CreateOnePlanetRecord($FleetRow['fleet_end_galaxy'], $FleetRow['fleet_end_system'], $FleetRow['fleet_end_planet'], $FleetRow['fleet_owner'], $lang['sys_occult_defaultname'], false);
				if ( $NewOwnerPlanet == true ) {
					$TheMessage = $lang['sys_occult_arrival'] . $TargetAdress;
					SendSimpleMessage ( $FleetRow['fleet_owner'], '0', $FleetRow['fleet_start_time'], 0, $lang['sys_occult_mess_from'], $lang['sys_occult_mess_report'], $TheMessage);
			...
			$TheMessage = $lang['sys_occult_arrival'] . $TargetAdress ;
			//SendSimpleMessage ( $FleetRow['fleet_owner'], '0', $FleetRow['fleet_end_time'], 0, $lang['sys_occult_mess_from'], $lang['sys_occult_mess_report'], $TheMessage);
			// Mettre a jour la flotte pour qu'effectivement elle revienne ! 
			if ( $FleetRow['fleet_end_time'] + 432000 <= time()  ) {   //Retour d'orbite 5 jours
			doquery("UPDATE {{table}} SET `fleet_mess` = '1' WHERE `fleet_id` = ". $FleetRow["fleet_id"], 'fleets');//A etait effacer pour la mise en orbite
				}
		}
	} else {
