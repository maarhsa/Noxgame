...
function FinishRechercheFromQueue ( &$CurrentPlanet,&$CurrentUser ) {
	...
	if ($CurrentQueue != 0) {
		...
		if ( $Element != false ) {
			...
			{
				$sous = MAX_FINISH_BONUS_TECH;
				// ici on ajoute 1 au niveau :)
				$calcule =  $CurrentUser[$resource[$Element]] + 1;
				...
				$QryUpdatevote .= "WHERE ";
				$QryUpdatevote .= "`id` = '" .$CurrentUser['id']. "';";
				doquery( $QryUpdatevote, 'users');
				
				$Estado = "Laboratoire";
				$Text = "".$user['username']." a finis la recherche :".$lang['tech'][$Element]." prematurement le ".date ( "d-m-Y H:i:s" , time() )." \n";
