...
function ItemBuildingPage ( &$CurrentPlanet, $CurrentUser ) {
	...
	if ($CurrentQueue != 0) 
	{
		...
		// for($search = 0;$search <= $compteurqueue;$search++)
		// {
			...
			// pour voir les elements dans la queue
			// var_dump($lelabo);
		// }
	...
	{
		if($CurrentUser['vote']>=3)
		{
			FinishItemFromQueue ($CurrentPlanet);
		}
		else
		{
			message("il vous faut 3  points votes au minimum","erreur de construction");
		}
	}
	...
	if (isset($_POST['fmenge'])) {
		...
		foreach($_POST['fmenge'] as $Element => $Count) {
			...
			if(in_array($Element, $reslist['item'])) 
			{
	...
	$TabIndex = 0;
		foreach($reslist['item'] as $Element) {
			if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element)) {
	...
	if ($CurrentPlanet['b_item_id'] != '') {
		$BuildQueue .= ItemBuildListBox( $CurrentUser, $CurrentPlanet );
	}
	...
	$page .= parsetemplate(gettemplate('Item/buildings_item'), $parse);
