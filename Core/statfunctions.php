...
function GetPertesPoints () {
	global $resource, $pricelist, $reslist,$user;

	$pertetotal = round($user['p_infligees']/1000);
	//si il est negative
	if($pertetotal<0)
	{
		$pertetotal = 0;
...
function GetFleetPoints ($CurrentPlanet,$CurrentUser) {
	global $resource, $pricelist, $reslist,$user;
	...
	$FleetPoints = 0;
		foreach($reslist['fleet'] as $n => $Fleet) 
		{
			if ($CurrentPlanet[ $resource[ $Fleet ] ] > 0) {
				...
				/*$us = intval($CurrentUser);
				//1.284.763.968.403.400
				// si il a des vaisseaux en vol
				var_dump($us);
				$fq = doquery("SELECT `fleet_array` FROM {{table}} WHERE fleet_owner='".$us."';", "fleets");
				while ($f = mysql_fetch_array($fq)) 
				{
					$flotten = explode(";", $f['fleet_array']);
					foreach ($flotten as $a => $b) 
					{
						if ($b != '') 
						{
							$a = explode(",", $b);
							//$a[0] == le nom
							//$a[1] == le nombre
							// var_dump($CurrentPlanet[ $resource[ $Fleet ] ]);
							
							$Units         = $pricelist[$a[0]]['metal'] + $pricelist[$a[0]]['crystal'] + $pricelist[$a[0]]['deuterium'];
							$FleetflyPoints   +=($Units * $a[1]);
							$FleetflyCounts   += $a[1];
...
/*function GetFleetflyPoints ( $CurrentPlanet) {
	global $resource, $pricelist, $reslist,$user;
        ...
        // si il a des vaisseaux en vol
            ...
            while ($f = mysql_fetch_array($fq)) 
            {
				...
				foreach ($flotten as $a => $b) {
								...
								//$a[1] == le nombre
								$Units         = $pricelist[$a[0]]['metal'] + $pricelist[$a[0]]['crystal'] + $pricelist[$a[0]]['deuterium'];
								$FleetPoints   +=($Units * $a[1]);
								$FleetCounts   += $a[1];
								}
						} 
            }
	
	$RetValue['FleetCount'] = $FleetPoints;
	$RetValue['FleetPoint'] = $FleetCounts;
